...
<body>
      ...
      <div class="col-md-12">
        <div class="control-group">
          <div class="form-group">
              <form action="<?= base_url() ?>index.php/presensi/insertabsen" method="post" class="form-vertical one-row" onsubmit="submitform(this);return false;">
  ...
  <button class="btn btn-danger" id="btn-telat">Telat</button>
  <script src="<?= libraries_url('jquery-ui.min.js'); ?>"></script>
  <script type="text/javascript">
    var available=[];
    <?php foreach ($mentee as $result) : ?>
    ...
    var find=function(e){
      //if(e.which===13) return false;
        ...
        if($(e).val()!=''){
          $("#msg").html("<img height='30px' src='<?=base_url('asset/css/images/ajax-loader.gif') ?>'/>");
          $.ajax({
    ...
    var submitform=function(e){
      ...
      $(".form-group").removeClass("has-warning");
      $(".form-group").removeClass("has-success");
      if($('body').hasClass('telat')){
        var hhh = $(e).attr('action') + "/yes";
        console.log("okee")
      }
      else{
        var hhh = $(e).attr('action');
      }
      $.ajax({
    ...
    <script src="<?= libraries_url('bootstrap.min.js'); ?>"></script>
